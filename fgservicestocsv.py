...
# This file is part of fgservicestocsv.
#
...
p_entering_group_block = re.compile('^\s*config firewall service ', re.IGNORECASE) #AngelOfTerror: Changed string to search for, now looks for "config firewall service *" which will match both the "config firewall service custom" and "config firewall service group" sections
				...
				for key in keys:
					if key in group.keys():
                                                if "member" == key:
													...
													output_line.append("|".join(group[key].split(" "))) #AngelOfTerror: I prefer all items on the same line, seperated by a pipe.
                                                else:
